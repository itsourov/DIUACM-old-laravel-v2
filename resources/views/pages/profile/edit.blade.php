<x-app-layout title="Edit Profile">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl lg:rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div class="p-4 md:p-6 lg:p-8">
                    ...
                    <div class="border-t border-slate-200 dark:border-slate-700 mt-8 pt-8">
                        ...
                        <form action="{{ route('my-account.password.update') }}" method="POST" id="passwordForm">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="flex justify-end space-x-4">
                                <x-button type="submit" class="w-full sm:w-auto">
                                    <span class="flex items-center justify-center">
                                        <svg class="animate-spin -ml-1 mr-3 h-4 w-4 text-white hidden" id="password-loading-spinner" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const passwordLoadingSpinner = document.getElementById('password-loading-spinner');
            ...
            passwordForm.addEventListener('submit', function(e) {
                ...
                if (password.length < 8) {
                    e.preventDefault();
                    showNotification('Password Error', 'Password must be at least 8 characters.', 'error');
                    return;
                }
                
                if (password !== passwordConfirmation) {
                    e.preventDefault();
                    showNotification('Password Error', 'Password confirmation does not match.', 'error');
                    return;
                }
                
                // Show loading state
                passwordLoadingSpinner.classList.remove('hidden');
                passwordSubmitText.textContent = 'Updating Password...';
